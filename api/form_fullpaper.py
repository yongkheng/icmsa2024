...
@router.post("/api/form_fullpaper")
async def form_fullpaper(request: Request, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    if resp.all():
        ...
        file_ext = form["submission_pdf_file"].filename.split('.')[-1]
        submission_filename = f"full_paper_{ts1}_{latest_record.submission_id}_rev{revision_version}.{file_ext}"
        ...
        new_rec = models.Submission(
            ...
            fullpaper_filename=submission_filename,
            fullpaper_content=form["submission_pdf_file"].file.read(),
        ...
        with open("receipt.html", 'wb') as f:
            f.write(html.body)
        new_rec.fullpaper_receipt = html.body
